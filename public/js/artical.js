function articalOk() {
    ...
    xhr.onreadystatechange = function () {
        ...
        if (xhr.readyState == 4) {
            ...
            if (info.errorNo == 0) {
                //添加成功，跳转到添加文章首页
                window.location.href = "index.php?p=admin&c=Artical&a=show";
            } else {
                //添加失败，提示信息
                alert(info.message);
            }
    ...
    xhr.open("post", "index.php?p=admin&c=Artical&a=addArticle", true);
    //转变成xml格式
...
function upArticalOk() {
    ...
    xhr.open("post",'index.php?p=admin&c=Artical&a=upArticle',true);
    //转变成xml格式
    xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhr.send(postData);
